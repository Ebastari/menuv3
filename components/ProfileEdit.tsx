...
export const ProfileEdit: React.FC<ProfileEditProps> = ({ user, onSave, onClose }) => {
  ...
  const handleVerifySecurity = () => {
    ...
    if (formData.oldAuthUser.toLowerCase() === storedUser.toLowerCase() && 
        formData.oldAuthPass === storedPass) {
      setSecurityVerified(true);
      setFormData(prev => ({ 
        ...prev, 
        newAuthUser: storedUser, 
        newAuthPass: storedPass 
      }));
    } else {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Jika security pernah dibuka dan diverifikasi, simpan kredensial baru
    if (securityVerified && formData.newAuthUser && formData.newAuthPass) {
      localStorage.setItem('montana_auth_user', formData.newAuthUser);
      localStorage.setItem('montana_auth_pass', formData.newAuthPass);
    }
